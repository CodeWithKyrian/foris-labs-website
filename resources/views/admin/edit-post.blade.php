<x-app-layout title="Edit Post">
    ...
    <div class="grid grid-cols-12 gap-6">
        <div x-data="{ tab: 1 }" class="col-span-full bg-white shadow-lg rounded-sm border border-ash-200">
            ...
            <div class="">
                <form class="px-5 py-2" enctype="multipart/form-data" action="{{route('admin.posts.update', ['post' => $post->id])}}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mt-4 text-gray-700">
                        <div class="py-2">
                            <label class="font-medium" for="title">Title</label>
                            <input type="text" name="title" value="{{$post->title}}" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-orange-500 focus:ring-0 focus:outline-none">
                        </div>
                        <div class="py-2">
                            <label class="font-medium" for="emailAddress">Summary</label>
                            <textarea rows="2" name="summary" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-orange-500 focus:ring-0 focus:outline-none">{{$post->summary}}</textarea>
                        </div>
                        <div class="py-2">
                            <label class="font-medium" for="body">Body</label>
                            <textarea id="editor" name="body" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-orange-500 focus:ring-0 focus:outline-none">
                            {{$post->body}}
                            </textarea>
                        </div>
                        <div class="py-2">
                            <label for="toogleButton" class="flex items-center cursor-pointer">
                                <div class="relative pr-4">
                                    <input id="toogleButton" @if($post->featured) checked @endif type="checkbox" name="featured" class="hidden peer"/>
                                ...
                                <div class="flex items-center justify-center w-full mx-auto pb-[56.25%] relative mt-2">
                                    <div class="absolute inset-0 overflow-hidden">
                                        <label for="banner" class="flex flex-col items-center justify-center w-full h-full border-2 border-ash-500 rounded border-dashed hover:bg-ash-200 hover:border-ash-600">
                                            <img id="preview" class="absolute block object-cover w-full" src="{{$post->image_url}}">
                                        ...
                                        <input type="file" class="hidden" id="banner" name="banner" accept="image/*" @change="showPreview(event)" />
                                    </div>
                                </div>
                            </div>
                        </div>
